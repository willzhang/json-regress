...
源码仓库：[willzhang/json-regress](https://github.com/willzhang/json-regress)。模块名为 `willzhang/json_regress`，版本 `0.1.0`。核心使用现有 `Json` 类型，无第三方运行时依赖；可选 ML 集成和 Native 文件 CLI 分别在独立模块中。
...
`.github/workflows/check.yml` 复用本地验证脚本；[首次远程 Actions](https://github.com/willzhang/json-regress/actions/runs/34912598554) 已在提交 `3620117` 通过。已配置本仓库署名并形成 10 个实质开发阶段提交，数量检查通过；详见 [开发历史](docs/DEVELOPMENT_HISTORY.md)。GitHub 仓库已公开，Mooncakes namespace 已确认为 `willzhang`；发布与报名交接见 [PUBLICATION](docs/PUBLICATION.md)。
