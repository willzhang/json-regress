...
moon_cmd run cmd/rl
moon_cmd -C cli fmt --check main
cli_binary="$(bash scripts/build-cli.sh)"
python3 scripts/check-cli.py --binary "$cli_binary"
if [[ ${1:-} == '--integration' ]]; then
  python3 scripts/fetch-moonxi.py
  moon_cmd -C integrations/moonxi fmt --check . cmd/showcases cmd/export
  mkdir -p artifacts
  integration_log="$project_dir/artifacts/moonxi-validation.log"
  if ! (
    set -e
    moon_cmd -C integrations/moonxi check . cmd/showcases cmd/export --target native
    moon_cmd -C integrations/moonxi check . cmd/showcases cmd/export --target wasm
    moon_cmd -C integrations/moonxi test -p local/json_regress_moonxi --target native
    moon_cmd -C integrations/moonxi test -p local/json_regress_moonxi --target wasm
  ...
  python3 scripts/check-ml-replay.py --binary "$cli_binary"
  python3 scripts/fetch-moonxi.py
