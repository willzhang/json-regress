...
## 2026-09-15 0.1.2 发行验证

发布源 `9a05dca9efdb0f63bee520b0fda38ed20a3fd5fd` 的 [CI](https://github.com/willzhang/json-regress/actions/runs/34963192482) 通过；远程使用 moonc v0.10.13，本地兼容验证使用 v0.10.12。完整库/真实集成/CLI/输出对照及重放均通过。发布返回 200 OK，100 个归档文件的检查无命中，可达历史 298 个提交/文件对象的检查无命中。

0.1.2 独立注册表安装成功，100 个文件与发布归档逐字节一致，Wasm/Native 各 3 项消费测试通过；未使用本地 workspace 替代注册表依赖。
